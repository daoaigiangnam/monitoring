...
export async function evaluateAlerts(db,hostId,metrics){
 ...
 for(const m of metrics){const value=Number(m.value);if(!Number.isFinite(value))continue;
  for(const r of rules){if(r.metric_key!==m.key)continue;const bad=matches(value,r.operator,Number(r.threshold));
   ...
   const [[state]]=await db.execute('SELECT first_bad_at FROM alert_rule_states WHERE host_id=? AND rule_id=?',[hostId,r.id]);
   const firstBad=state?.first_bad_at?new Date(state.first_bad_at):now;
